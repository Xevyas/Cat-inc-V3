...
- An obstacle can be removed only when its territory is already claimed and at least one cell on the outside edge of its complete footprint touches clear land orthogonally. One action clears every cell occupied by that obstacle. Clearing is instant and free in the prototype; future resource costs, worker requirements and timers belong in gameplay data rather than rendering code.
...
- The Sawmill, Catchen and Pawsonry action menu uses a 46 px round target. It remains above the tapped footprint and does not participate in map panning or long-press selection.
