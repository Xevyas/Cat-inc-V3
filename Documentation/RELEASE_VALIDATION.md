...
The final V3 Legacy build restores the v0.0037 gameplay tree and adds only its permanent retirement notice and technical update channel.

- Release status: final public build for game version v0.0037 Legacy.
- Technical update manifest: `0.0039`, deliberately newer than the previously served v0.0038 build.
- Interface references: CSS `0.0116`, JavaScript `0.0132`, production module `0.0036`, save module `0.0042`, state module `0.0040`, audio module `0.0007`, config data `0.0043`, content data `0.0052`.
- Automated suite: 177/177 tests passing.
- Browser load: desktop Chrome at 1440 × 900 and mobile Chrome at 390 × 844 show the permanent Legacy banner without document overflow, retain Settings version v0.0037, expose no Camp tab and load manifest version 0.0039. The only blocked browser request during local validation was the external Google Fonts stylesheet; no JavaScript exception or missing local runtime asset occurred.
...
Required result: zero failed, cancelled, skipped or todo tests (177 tests currently).
